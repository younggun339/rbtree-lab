...
//  #endif
 ...
 void test_find_single(rbtree *t, const key_t key, const key_t wrong_key) {
//    rbtree *t = new_rbtree();
   node_t *p = rbtree_insert(t, key);
 ...
 void test_erase_root(rbtree *t, const key_t key) {
//    rbtree *t = new_rbtree();
   node_t *p = rbtree_insert(t, key);
   visualize_tree(t, 4);
   assert(p != NULL);
 ...
 void test_find_erase(rbtree *t, const key_t *arr, const size_t n) {
   ...
   for (int i = 0; i < n; i++) {
    ...
    //  visualize_tree(t,4);
     ...
     rbtree_erase(t, p);
     visualize_tree(t,4);
    //  printf("%p, %d, %p, %p\n", t->root,t->root->key, t->root->left, t->root->right);
    //  printf("%d, %p\n",t->nil->key, t->nil);
    //  visualize_tree(t,4);
   }
...
//    }
 ...
 void test_find_erase_fixed(rbtree *t) {
   const key_t arr[] = {5,34, 67, 23, 156, 24, 36,990, 25};
...
//    visualize_tree(t,4);
//    printf("%d", 3);
//    test_insert_single(t, 1024);
//    visualize_tree(t,5);
   visualize_tree(t,4);
//    test_find_single(t, 512, 1024);
  //  test_erase_root(t, 128);
   visualize_tree(t,4);
   test_find_erase_fixed(t);
  //  visualize_tree(t,4);
